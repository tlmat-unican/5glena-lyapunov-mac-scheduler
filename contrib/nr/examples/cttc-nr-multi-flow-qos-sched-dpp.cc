...
main(int argc, char* argv[])
{
    ...
    qosUe1flow.gbrDl = 0e6; // Downlink GBR

    // The bearer that will carry Ue 2 Flows DC GBR traffic
    EpsBearer ue1flowDcGbrBearer(EpsBearer::NGBR_LOW_LAT_EMBB, qosUe1flow);
    ...
    qosUe2flow.gbrDl = 2e6; // Downlink GBR

    // The bearer that will carry Ue 2 Flows DC GBR traffic
    EpsBearer ue2flowDcGbrBearer(EpsBearer::NGBR_LOW_LAT_EMBB, qosUe2flow);
    ...
    qosUe3flow.gbrDl = 10e6; // Downlink GBR

    // The bearer that will carry Ue 2 Flows DC GBR traffic
    EpsBearer ue3flowDcGbrBearer(EpsBearer::GBR_GAMING, qosUe3flow);
    ...
    */
         ...
         ++i)
    {
        ...
        if (i->second.rxPackets > 0)
        {
            ...
            outputFile << throughput << "\t" << i->second.rxBytes * 8.0 << "\t" << flowDuration << "\t" << i->second.txBytes * 8.0 / flowDuration / 1000.0 / 1000.0 << std::endl;
