        ...
        for (const auto& ueLcg : ue.first->m_dlLCG)
        {
            ...
            for (const auto lcId : ueActiveLCs)
            {
                ...
                if (LCPtr->m_resourceType == LogicalChannelConfigListElement_s::QBT_DGBR)
                {
                    ...
                    // std::cout << "\tEste es DC-GBR" << std::endl;
                }
